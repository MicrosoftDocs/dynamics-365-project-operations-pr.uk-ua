...
## <a name="prerequisites-if-you-use-dynamics-365-finance"></a>Обов'язкові умови, якщо ви використовуєте Dynamics 365 Finance
...
- [Для Android телефонів](https://go.microsoft.com/fwlink/?linkid=850662)
...
## <a name="approve-an-expense-report-by-using-the-expense-management-mobile-workspace"></a>Затвердження звіту про витрати за допомогою мобільної робочої області керування витратами
...
## <a name="create-a-new-expense-report-and-submit-it-for-approval-by-using-the-expense-management-mobile-workspace"></a>Створіть новий звіт про витрати та надішліть його на затвердження за допомогою мобільної робочої області керування витратами
...
### <a name="why-doesnt-the-expense-mobile-app-enter-the-payment-method-by-default"></a>Чому мобільний додаток Expense не вводить спосіб оплати за умовчанням?

Організації можуть настроїти **параметр спосіб оплати за промовчанням для кожної категорії витрат під час її** створення. Крім того, під час настроювання способів оплати можна встановити **поле спосіб оплати за промовчанням лише** **імпорт**.

Якщо **ввімкнуто лише імпорт для способу** оплати, спосіб оплати не вводиться за промовчанням. Він буде порожнім у категоріях витрат, де налаштовано цей спосіб оплати. Така поведінка узгоджується як у веб-досвіді, так і в мобільному досвіді.
    
Якщо **лише імпорт не ввімкнуто для способу** оплати, встановлене значення вводиться за промовчанням для категорій витрат, де настроєно цей спосіб оплати. Однак, існує відома проблема, коли значення за промовчанням не вводиться в мобільний додаток витрати. Щоб вирішити цю проблему, вручну виберіть спосіб оплати, перш ніж зберегти звіт про витрати. 

### <a name="why-cant-i-add-or-edit-financial-dimensions-in-the-expense-mobile-app"></a>Чому не вдається додати або змінити фінансові розміри в мобільному додатку Expense?

Введення параметрів і розподілів не підтримується. Щоб вирішити це обмеження, ці поля можна встановити за промовчанням у мобільному додатку, встановивши фінансові параметри за промовчанням для одного проекту або працівника.

### <a name="why-do-i-sometimes-see-a-synchronization-error-in-the-expense-mobile-app"></a>Чому іноді в мобільному додатку Expense з'являється помилка синхронізації?

Якщо витрати рядків не відповідають вимогам політики, і користувач надсилає звіт про витрати без вирішення попередження політики, мобільні дані не синхронізуються з сервером, і виникає помилка синхронізації. Усі звіти про витрати, надіслані після того, як сталася помилка синхронізації, залишаться в стані помилки та спричинять більше помилок синхронізації. Єдиний спосіб вирішити цю ситуацію - вручну видалити сповіщення про синхронізацію. Цю проблему було вирішено, зупинивши подання звітів про витрати, коли попередження політики не було вирішено, щоб уникнути помилок синхронізації.
...
Ця перевірка наразі не підтримується. Однак у майбутньому може бути додана підтримка. 

### <a name="what-document-types-are-supported-in-the-expense-mobile-app"></a>Які типи документів підтримуються в мобільному додатку Expense?
